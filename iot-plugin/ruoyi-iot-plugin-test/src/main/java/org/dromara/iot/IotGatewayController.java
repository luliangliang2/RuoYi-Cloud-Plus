...
public class IotGatewayController {
	...
	private final List<ScriptEngineProvider> scriptEngines;

	public IotGatewayController(DeviceRegistry deviceRegistry, DeviceRegistryAdmin deviceRegistryAdmin,
			SessionRepository sessionRepository, DeviceMessageBus messageBus, PluginRegistry pluginRegistry,
			...
			ObjectProvider<GatewayNodeCoordinator> clusterCoordinator, ConfigurationRuntime configurationRuntime,
			...
			ObjectProvider<EtcdConfigurationCenterProperties> etcdProperties, PluginServiceRegistry pluginServices,
			HandshakeProviderRegistry handshakeProviders, HandshakeDebugService handshakeDebug,
			ObjectProvider<ExternalPluginManager> externalPlugins, ScriptRegistry scriptRegistry,
			ScriptRuntime scriptRuntime, List<ScriptEngineProvider> scriptEngines) {
		...
		this.scriptEngines = List.copyOf(scriptEngines);
	}

	@GetMapping("/status")
	public Map<String, Object> status() {
		var providerHealth = providerHealthCatalog.snapshots();
		boolean providersUp = providerHealth.stream().allMatch(
				snapshot -> snapshot.health().status() == org.ssssssss.magicapi.iot.plugin.api.PluginHealth.Status.UP);
		return Map
				.of("status", providersUp ? "UP" : "DOWN", "deviceRegistry", deviceRegistry.getClass().getSimpleName(),
						"sessionRepository", sessionRepository.getClass().getSimpleName(), "messageBus",
						messageBus.getClass().getSimpleName(), "configurationCenter", configurationRuntime.providerId(),
						"pluginCount", pluginRegistry.snapshots().size(), "capabilityCount",
						capabilityRegistry.capabilities().size(), "providerCount", providerHealth.size(),
						"healthyProviderCount",
						providerHealth.stream()
								.filter(snapshot -> snapshot.health()
										.status() == org.ssssssss.magicapi.iot.plugin.api.PluginHealth.Status.UP)
								.count());
	}
	...
	public Map<String, Object> scripts() {
		return Map.of("scripts", scriptRegistry.list(), "engines", scriptEngines.stream().map(engine -> Map.of("id",
				engine.engineId(), "serviceId", engine.serviceId(), "languages", engine.languages())).toList(),
				"modules", ScriptModuleCatalog.defaults());
	}

	@GetMapping("/scripts/{scriptId}")
	public ScriptDefinition script(@PathVariable String scriptId) {
		return scriptRegistry.find(scriptId)
				.orElseThrow(() -> new IllegalArgumentException("Script not found: " + scriptId));
	}

	@PutMapping("/scripts/{scriptId}")
	public Map<String, Object> saveScript(@PathVariable String scriptId, @RequestBody ScriptWrite request) {
		ScriptDefinition definition = new ScriptDefinition(scriptId, requireText(request.name(), "name"),
				requireText(request.engineId(), "engineId"), requireText(request.language(), "language"),
				request.source(), scriptRegistry.find(scriptId).map(item -> item.version() + 1).orElse(1),
				ScriptDefinition.Status.DRAFT, false, request.permissions(), request.triggers(),
				java.time.Duration.ofMillis(request.timeoutMs()), request.metadata());
		ScriptCompileResult validation = scriptRuntime.validate(definition);
		ScriptDefinition saved = scriptRegistry.save(definition);
		return Map.of("script", saved, "validation", validation);
	}

	@PostMapping("/scripts/{scriptId}/validate")
	public ScriptCompileResult validateScript(@PathVariable String scriptId) {
		return scriptRuntime.validate(script(scriptId));
	}

	@PostMapping("/scripts/{scriptId}/publish")
	public ScriptDefinition publishScript(@PathVariable String scriptId) {
		return scriptRegistry.publish(scriptId);
	}

	@PutMapping("/scripts/{scriptId}/enabled")
	public ScriptDefinition enableScript(@PathVariable String scriptId, @RequestBody ScriptEnabled request) {
		return scriptRegistry.setEnabled(scriptId, request.enabled());
	}

	@PostMapping("/scripts/{scriptId}/rollback")
	public ScriptDefinition rollbackScript(@PathVariable String scriptId, @RequestBody ScriptRollback request) {
		return scriptRegistry.rollback(scriptId, request.version());
	}
	...
	@PostMapping("/scripts/{scriptId}/debug")
	public ScriptExecutionResult debugScript(@PathVariable String scriptId, @RequestBody ScriptDebug request) {
		return scriptRuntime.execute(scriptId, new ScriptExecutionContext(request.traceId(), request.trigger(),
				request.input(), request.attributes(), true));
	}
	...
	public RegisteredDevice device(@PathVariable String productId, @PathVariable String deviceId) {
		return deviceRegistry.find(new DeviceIdentity(productId, deviceId)).orElseThrow(
				() -> new IllegalArgumentException("Device is not registered: " + productId + "/" + deviceId));
	}
	...
	public Map<String, Object> deviceCredentials(@PathVariable String productId, @PathVariable String deviceId) {
		...
		return Map.of("productId", productId, "deviceId", deviceId, "credentialTypes",
				deviceRegistryAdmin.credentialTypes(identity));
	}
	...
	public Map<String, Object> setDeviceCredential(@PathVariable String productId, @PathVariable String deviceId,
		...
		return Map.of("credentialTypes", deviceRegistryAdmin.credentialTypes(identity), "generatedCredential",
				generatedCredential);
	}
	...
	public Map<String, Object> verifyDeviceCredential(@PathVariable String productId, @PathVariable String deviceId,
		...
		DeviceIdentity identity = new DeviceIdentity(productId, deviceId);
		requireDevice(identity);
		boolean verified = deviceRegistryAdmin.verifyCredential(identity, new DeviceCredential(
				requireText(request.type(), "credentialType"), requireText(request.value(), "credential")));
		return Map.of("verified", verified);
	}

	@GetMapping("/runtime")
	public Map<String, Object> runtime() {
		return Map.of("protocol", protocolRuntime.snapshot(), "transports",
				transports.stream().map(ObservableTransportProvider::snapshot).toList());
	}

	@GetMapping("/cluster")
	public Map<String, Object> cluster() {
		if (clusterCoordinator == null)
			return Map.of("enabled", false, "nodes", List.of());
		return Map.of("enabled", true, "snapshot", clusterCoordinator.snapshot(), "nodes",
				clusterCoordinator.activeNodes());
	}
	...
	public Map<String, Object> configurationMemory() {
		return Map.of("provider", configurationRuntime.providerId(), "count", configurationRuntime.list("").size(),
				"values", configurationRuntime.list(""), "parsed", configurationRuntime.parsedSnapshots(),
	...
	public Map<String, Object> configurationMeta() {
		...
		case "nacos" -> {
			...
			yield p == null ? Map.of()
					: Map.of("serverAddr", p.getServerAddr(), "namespace", p.getNamespace(), "dataId", p.getDataId(),
							"group", p.getGroup());
		}
		case "zookeeper" -> {
	...
	public Map<String, Object> components() {
		List<Map<String, Object>> items = pluginRegistry.snapshots().stream().map(this::componentView).toList();
		return Map.of("count", items.size(), "capabilityCount", capabilityRegistry.capabilities().size(),
				"capabilities", capabilityRegistry.capabilities(), "components", items);
	}

	@GetMapping("/spi")
	public Map<String, Object> spi() {
		var services = pluginServices.snapshots();
		return Map.of("count", services.size(), "services", services, "handshakeProviders",
				handshakeProviders.handshakes().stream()
						.map(provider -> Map.of("id", provider.serviceId(), "protocols", provider.supportedProtocols()))
						.toList(),
				"authenticators", handshakeProviders.authenticators().stream()
						.map(authenticator -> Map.of("id", authenticator.serviceId())).toList());
	}
	...
	public Map<String, Object> spiMatches(@RequestParam String protocolId) {
		var candidates = handshakeProviders.handshakes().stream()
				...
				.map(provider -> Map.of("id", provider.serviceId(), "protocols", provider.supportedProtocols()))
				.toList();
		return Map.of("protocolId", protocolId, "count", candidates.size(), "candidates", candidates, "conflict",
				candidates.size() > 1);
	}

	@PostMapping("/spi/handshake/debug")
	public HandshakeDebugService.DebugResult debugHandshake(@RequestBody HandshakeDebugService.DebugRequest request) {
	...
	public Map<String, Object> externalPlugins() {
		if (externalPlugins == null)
			return Map.of("enabled", false, "plugins", List.of(), "errors", Map.of());
		return Map.of("enabled", true, "directory", externalPlugins.pluginDirectory().toString(), "plugins",
				externalPlugins.snapshots(), "errors", externalPlugins.discoveryErrors());
	}
	...
	private ExternalPluginManager requireExternalPlugins() {
		if (externalPlugins == null)
			throw new IllegalStateException("External plugin loading is disabled");
		return externalPlugins;
	}
	...
	private Map<String, Object> componentView(PluginSnapshot snapshot) {
		...
		String module = descriptor.capabilities().isEmpty() ? "plugin"
				: descriptor.capabilities().get(0).split(":", 2)[0];
		var providerHealth = providerHealthCatalog.snapshots().stream().filter(
				provider -> descriptor.capabilities().contains(provider.providerType() + ":" + provider.providerId()))
				.findFirst();
		boolean providerPlugin = descriptor.capabilities().stream().map(capability -> capability.split(":", 2)[0])
	...
	public record DeviceRegistration(String productId, String deviceId, boolean enabled, String protocolId,
			Map<String, String> capabilities, String credentialType, String credential, boolean generateCredential) {
	}
	...
	public record ScriptWrite(String name, String engineId, String language, String source, Set<String> permissions,
			Set<String> triggers, long timeoutMs, Map<String, Object> metadata) {
	}

	public record ScriptEnabled(boolean enabled) {
	}

	public record ScriptRollback(int version) {
	}

	public record ScriptDebug(String traceId, String trigger, Map<String, Object> input,
