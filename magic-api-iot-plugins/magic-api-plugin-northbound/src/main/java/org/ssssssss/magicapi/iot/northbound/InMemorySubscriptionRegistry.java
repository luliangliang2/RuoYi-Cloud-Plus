...
public class InMemorySubscriptionRegistry implements SubscriptionRegistry {
    ...
    public Collection<Subscription> find(String tenantId, String topic) {
        return subscriptions.values().stream().filter(s -> s.tenantId().equals(tenantId) && (s.topics().contains("*") || s.topics().contains(topic))).toList();
    }
}
