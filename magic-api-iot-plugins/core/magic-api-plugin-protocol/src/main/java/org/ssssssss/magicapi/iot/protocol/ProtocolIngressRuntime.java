...
public final class ProtocolIngressRuntime implements SmartLifecycle, TransportProvider.TransportMessageHandler {
	...
	public ProtocolIngressRuntime(ProtocolPipelineRegistry pipelines, DeviceMessageBus messageBus,
			List<TransportProvider> transports, HandshakeCoordinator handshakes, TelemetryRecorder telemetry) {
	...
	public void received(String connectionId, ByteBuffer payload, ProtocolContext context) {
		...
		long started = System.nanoTime();
		try {
			HandshakeCoordinator.Decision decision = handshakes.received(connectionId, payload.asReadOnlyBuffer(),
					context);
			if (decision.response().hasRemaining())
				transport(context.transport())
						.ifPresent(transport -> transport.send(connectionId, decision.response()));
			if (decision.close()) {
				transport(context.transport()).ifPresent(transport -> transport.disconnect(connectionId));
				return;
			}
	...
	public void received(String connectionId, DeviceMessage message, ProtocolContext context) {
		...
		try {
			...
			telemetry.record("iot.provider.latency", elapsed(started),
					tags("message-bus", context.transport(), "done"));
		}
	}
	...
	public ProtocolRuntimeSnapshot snapshot() {
		return new ProtocolRuntimeSnapshot(isRunning(), pipelines.protocolIds(), transports.size(),
				activeConnections.get(), receivedFrames.get(), publishedMessages.get(), unsupportedFrames.get(),
				errors.get());
	}
	...
	private static Map<String, String> tags(String provider, String transport, String status) {
		return Map.of("provider", provider == null ? "unknown" : provider, "transport",
				transport == null ? "unknown" : transport, "status", status);
	}
}
