<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="就职时长" prop="lengthOfEmployment">
        <el-input
          v-model="queryParams.lengthOfEmployment"
          placeholder="请输入就职时长"
          clearable
          @keyup.enter.native="handleQuery"
        />
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['admin:user-work-experience:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['admin:user-work-experience:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['admin:user-work-experience:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['admin:user-work-experience:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="userWorkExperienceList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="离职时间" align="center" prop="departureTime" width="180">
        <template slot-scope="scope">
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="就职时长" prop="lengthOfEmployment">
          <el-input v-model="form.lengthOfEmployment" placeholder="请输入就职时长" />
        </el-form-item>
        <el-form-item label="说明介绍" prop="introduction">
...
<script>
import { listUserWorkExperience, getUserWorkExperience, delUserWorkExperience, addUserWorkExperience, updateUserWorkExperience } from "@/api/admin/user-work-experience";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        lengthOfEmployment: [
          { required: true, message: "就职时长不能为空", trigger: "blur" }
        ],
        introduction: [
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listUserWorkExperience(this.queryParams).then(response => {
        this.userWorkExperienceList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getUserWorkExperience(id).then(response => {
        this.loading = false;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          if (this.form.id != null) {
            updateUserWorkExperience(this.form).then(response => {
            ...
            addUserWorkExperience(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除工作经历编号为"' + ids + '"的数据项？').then(() => {
        this.loading = true;
        return delUserWorkExperience(ids);
      }).then(() => {
        this.loading = false;
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('admin/user-work-experience/export', {
        ...this.queryParams
      }, `user-work-experience_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
