...
1. rabbit: 普通消息、延迟队列
2. rocket：原生实例包含--普通消息、事务消息；Springboot实例--包含普通消息、事务消息
3. kafka：普通消息、stream流(待实现)
...
│      │                  │      │      
│      │                  │      └─SpringInstances SpringBoot实例 消费者
...
│      │                          └─SpringInstances SpringBoot实例 生产者
...
在RocketMQ 5原生实例中；实现的事务消息这部分；在发送半事务的时候需要进行**延时操作**，这样可以是代码进入到本地事务的执行和check中。
